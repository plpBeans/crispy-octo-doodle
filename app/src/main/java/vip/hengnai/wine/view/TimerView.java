...
public class TimerView extends LinearLayout {
    ...
    public TimerView(Context context, AttributeSet attr) {
        super(context, attr);
        TypedArray attrs = context.obtainStyledAttributes(attr, R.styleable.timerView);
        textColor = attrs.getColor(R.styleable.timerView_textColor, Color.parseColor("#666666"));
        init(context);
    }
    ...
    public void init(Context context) {
        ...
        //初始化的时候清空view和map
        // 获取键集合的迭代器
        Iterator it = viewPool.keySet().iterator();
        while (it.hasNext()) {
            removeView(viewPool.get(it.next()));
        }
        viewPool.clear();
    ...
    private void show(long seconds) {
        ...
        if (seconds > 0) {
            ...
            if (viewPool.get(SECOND_TAG) != null && seconds == 0) {
                removeView(viewPool.get(SECOND_TAG));
                //回调
