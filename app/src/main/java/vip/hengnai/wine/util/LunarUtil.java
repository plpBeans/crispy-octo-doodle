...
import vip.hengnai.wine.entity.Lunar;
...
public class LunarUtil {
    ...
    public static int daysInLunarMonth(int year, int month) {
        int color=0x100000;
        if ((LUNAR_INFO[year - MIN_YEAR] & (color >> month)) == 0){
            return 29;
        }else{
            return 30;
        }
    ...
    private static String getDrawStr(int lunatMonth, int lunatDay, boolean isLeap) {
        ...
        String one="初一";
        int value10=10;
        if (one.equals(relust) && isLeap) {
            relust = "闰" + CHINESE_NUMBER[lunatMonth - 1] + "月";
        } else if (one.equals(relust) && !isLeap) {
            relust = CHINESE_NUMBER[lunatMonth - 1] + "月";
        } else if (lunatDay == value10) {
            relust = "初十";
        } else {
