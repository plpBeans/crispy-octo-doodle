 ...
 * BaseCharacterEncoder and BaseCharacterDecoder classes, one complete 57 * chunk of
 ...
 * CeFormatException 82 * @see BaseCharacterEncoder 83 * @see UCDecoder 84 * @see
 * UUDecoder 85 * @see BASE64Decoder 86
 */

public abstract class BaseCharacterDecoder
{

	/**
	 *the number of bytes per atom of decoding
	 * @return
	 */
	abstract protected int bytesPerAtom();

	/**
	 *the maximum number of bytes that can be encoded per line
	 * @return
	 */
	 ...
	 * decoding 149 * @exception CeStreamException The input stream is
	...
	{
		char[] inputBuffer = new char[inputString.length()];
		ByteArrayInputStream inStream;
		ByteArrayOutputStream outStream;

		inputString.getChars(0, inputString.length(), inputBuffer, 0);
		inStream = new ByteArrayInputStream(new String(inputBuffer).getBytes("UTF-8"));
