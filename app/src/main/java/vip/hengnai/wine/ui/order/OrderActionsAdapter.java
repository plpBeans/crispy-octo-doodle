 ...
 * @author Hugh
 */
public class OrderActionsAdapter extends RecyclerView.Adapter<OrderActionsAdapter.Holder> {


    private List<OrderEntity.ActionsBean> actionsBeanList;
    private Context context;

    public OrderActionsAdapter(Context context, List<OrderEntity.ActionsBean> actionsBeanList) {
    ...
    public interface OnClickListener {
         ...
         * @param actionsBean
         */
        void onItemClick(OrderEntity.ActionsBean actionsBean);
    }

    @Override
    public Holder onCreateViewHolder(ViewGroup parent, int viewType) {
        //加载布局文件
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_order_actions, parent, false);
    ...
    public void onBindViewHolder(final Holder holder, final int position) {
        ...
        if (Constants.VALUE_1.equals(actionsBeanList.get(position).getType())||Constants.VALUE_19.equals(actionsBeanList.get(position).getType()) ) {
            holder.btn.setBackgroundResource(R.mipmap.btn_zhifu);
            holder.btn.setTextColor(Color.WHITE);
        }else{
            holder.btn.setBackgroundResource(R.mipmap.btn_anniu);
